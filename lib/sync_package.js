...
var compare = function (id, callback) {
  ...
  request.get({
    ...
    if (dependencies.length > 0) {
      console.log("Check " + id + "'s " + ("" + dependencies.length).magenta + " dependencies");
...
var sync = function (id, callback) {
  ...
  npm.load('~/.npmrc', function (err) {
    ...
    console.log('Sync package "' + id + '" from ' + options.from + ' to ' + options.to.replace(/\/\/(.*@)/, '//'));
    compare(id, function (err, from, to) {
      if (err) {
        throw err;
      }
      console.log('The remote package "' + id.magenta + '"\' revsion is ' + from);
      console.log('The local package "' + id.magenta + '"\' revsion is ' + to);
      if (!from) {
